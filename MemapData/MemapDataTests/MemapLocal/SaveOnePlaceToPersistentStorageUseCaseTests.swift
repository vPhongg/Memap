//
//  SaveOnePlaceToPersistentStorageUseCaseTests.swift
//  MemapDataTests
//
//  Created by Vu Dinh Phong on 27/02/2026.
//

import XCTest
import MemapData

final class SaveOnePlaceToPersistentStorageUseCaseTests: XCTestCase {
    
    func test_init_doesNotMessageStoreUponCreation() {
        let (_, store) = makeSUT()
        
        XCTAssertEqual(store.receivedMessages, [])
    }
    
    func test_save_succeededSaveInsertion() async throws {
        let place = uniquePlace()
        let (sut, store) = makeSUT()
        try await sut.save(place)
        
        XCTAssertEqual(store.receivedMessages, [.insert(place.toLocal())])
    }
    
    func test_save_failsOnInsertionError() async throws {
        let place = uniquePlace()
        let (sut, store) = makeSUT()
        store.insertionResult = .failure(anyNSError())
        
        do {
            try await sut.save(place)
            expectFailure()
        } catch {
            XCTAssertEqual(error as NSError, anyNSError())
        }
    }
    
    
    // MARK: Helpers
    
    private func makeSUT(file: StaticString = #filePath, line: UInt = #line) -> (sut: LocalPlaceManager, store: MemapStoreSpy) {
        let store = MemapStoreSpy()
        let sut = LocalPlaceManager(store: store)
        trackForMemoryLeaks(store, file: file, line: line)
        trackForMemoryLeaks(sut, file: file, line: line)
        return (sut, store)
    }
}
